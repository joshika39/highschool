...
/*** Loaders import ***/
import {PLYLoader} from "./threejs/loaders/PLYLoader.js";
...
/*** Stats ***/
import Stats from './threejs/lib/stats.module.js';
...
let textMesh;
let cinemaVideo, videoTexture, movieMaterial, movieMesh, canPlay = false;
...
let functionNamespace = {
    ...
    setupScene: function setUpScene() {
        ...
        // scene.add(new THREE.AmbientLight('#cfcfcf', 0.2))
    },
    ...
    load3DObjects: function () {
        ...
        gLoader.load('assets/3D_Objects/cinema.glb', function (gltf) {
            ...
            cinema.position.set(box.max.x + Math.abs(box2.min.x) / 2 , -80, 0);
            scene.add(cinema);
    ...
    video: function videoSetup() {
        ...
        cinemaVideo.src = 'https://www.youtube.com/watch?v=z1klJIyx060';
        cinemaVideo.crossOrigin = "anonymous";
        cinemaVideo.load();

        previewVideo.src = 'assets/video/sao1.mp4';
        ...
        pVideoTexture = new THREE.VideoTexture(previewVideo);
        // videoTexture.needsUpdate = true;
        pMovieMaterial = new THREE.MeshBasicMaterial({
            map: pVideoTexture,
            side: THREE.DoubleSide
        })
        ...
        const onKeyUp = function (event) {
            ...
            switch (event.code) {
                ...
                case 'KeyE':
                    ...
                    break;
                case 'KeyR':
                    selectMoveNamespace.hideNShowPreview(true);
                    break;
...
let selectMoveNamespace = {
        ...
        for (let i = previewUI.firstPanel; i < previewUI.lastPanel; i++) {
            if (i === selectedMovieId) {
                ...
                if (!previewChanged) {
                    ...
                    previewPanel[i].material = pMovieMaterial;
                    previewChanged = true;
                }
            } else {
    ...
    hideNShowPreview: function hideNShowPreview(openTest) {
        if (isMovieMenuOpen) {
            for (let i = previewUI.firstPanel; i < previewUI.lastPanel; i++) {
                showingPreviewPanel[i].visible = false;
            }
            isMovieMenuOpen = false;
        } else {
            if (openTest) {
                for (let i = previewUI.firstPanel; i < previewUI.lastPanel; i++) {
                    showingPreviewPanel[i].visible = true;
                }
